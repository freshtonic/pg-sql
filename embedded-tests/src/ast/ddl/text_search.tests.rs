...
mod tests {
    ...
    fn parse_drop_text_search_configuration() {
        ...
        let stmt_parsed = DropTextSearchStmt::parse(&mut input).unwrap();
    ...
    fn parse_create_text_search_dictionary() {
        let lexed = crate::lex("CREATE TEXT SEARCH DICTIONARY alt_ts_dict1 (template=simple)");
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let _stmt_parsed = CreateTextSearchStmt::parse(&mut input).unwrap();
    ...
    fn parse_alter_text_search_configuration_rename() {
        ...
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let _stmt_parsed = AlterTextSearchStmt::parse(&mut input).unwrap();
        let _stmt = _stmt_parsed.ast();
        assert!(input.is_eof());
    }
    ...
    fn parse_drop_text_search_parser() {
        ...
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let _stmt_parsed = DropTextSearchStmt::parse(&mut input).unwrap();
        let _stmt = _stmt_parsed.ast();
        assert!(input.is_eof());
    }
    ...
    fn parse_create_text_search_dictionary_structured() {
        ...
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let stmt_parsed = CreateTextSearchStmt::parse(&mut input).unwrap();
        let stmt = stmt_parsed.ast();
    ...
    fn parse_create_text_search_configuration() {
        let lexed = crate::lex("CREATE TEXT SEARCH CONFIGURATION ispell_tst (PARSER = default)");
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let stmt_parsed = CreateTextSearchStmt::parse(&mut input).unwrap();
        let stmt = stmt_parsed.ast();
