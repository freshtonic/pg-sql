...
9. **An admission set must mirror the `gram.y` nonterminal it stands for.** Widening one to make a case parse is how both over-acceptance and slow dispatch arrive: a qualifier declared as a label set rather than `ColId` propagated through `Expr`, which heads every target list, and made the select head's FIRST 485 word kinds wide — `union.a` parsed as a column reference and the head's decision fell to the greedy path. When PostgreSQL says `ColId`, write `ColId`. These stay invisible until someone hits an `RCA4102 PrattAmbiguity` or measures a selector.
