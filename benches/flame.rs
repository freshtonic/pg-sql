...
fn run_loop(inputs: &[String], duration: Duration) -> LoopStats {
    ...
    'run: loop {
        for sql in inputs {
            stats.accepted += u64::from(std::hint::black_box(parse_with_pg_sql(sql)));
            stats.statements += 1;
...
fn main() -> ExitCode {
    ...
    let stats = run_loop(&inputs, Duration::from_secs(args.duration_secs));
