...
pub struct CreateTypeComposite<'input> {
    #[sep(COMMA)]
    pub columns: recursa::ArenaVec<'input, CompositeTypeColumn<'input>>,
}
...
pub struct CreateTypeEnum<'input> {
    #[sep(COMMA)]
    pub labels: recursa::ArenaVec<'input, literal::StringLit<'input>>,
}
...
pub struct AlterTypeCmdList<'input> {
    #[sep(COMMA)]
    pub cmds: recursa::ArenaVec1<'input, AlterTypeCmd<'input>>,
}
