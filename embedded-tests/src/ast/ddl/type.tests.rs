...
mod tests {
    ...
    fn parse_drop_type_list() {
        ...
        let stmt_parsed = DropTypeStmt::parse(&mut input).unwrap();
    ...
    fn parse_alter_type_rename_to() {
        ...
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let _stmt_parsed = AlterTypeStmt::parse(&mut input).unwrap();
    ...
    fn parse_create_type_shell() {
        ...
        assert_eq!(lexed.errors().count(), 0, "lex errors in input");
        let mut input = lexed.input();
        let stmt_parsed = CreateTypeStmt::parse(&mut input).unwrap();
        let stmt = stmt_parsed.ast();
