...
| Precedence instead of grammar levels | yes, since 2026-09-16 — one declared `precedence { ... }` block mirroring `gram.y:829-908`, with `#[parse(prec = ...)]` per rule. `Expr` is an ordinary left-recursive enum; the Pratt authoring surface is gone. |
...
| Nonterminals | 728 | 3,437 | 4.72x |
| Rules | 3,408 | 12,070 | 3.54x |
| LR states | 6,458 | 17,881 | 2.77x |
| Packed action/goto slots | 123,278 | 211,919 | 1.72x |

The pg-sql column is the declared-precedence machine, measured 2026-09-16 and
recorded in [`docs/metrics/lr-statistics-v2.txt`](../metrics/lr-statistics-v2.txt).
Two earlier machines are referenced throughout this section:

| Machine | Nonterminals | Rules | LR states | Packed slots |
| --- | --: | --: | --: | --: |
| v1, before Recursa inlined attachment units (`lr-statistics-v1.txt`) | 4,279 | 14,944 | 20,715 | 262,296 |
| Unchanged pg-sql on the declared-precedence Recursa revision | 3,659 | 14,333 | 20,191 | — |
| v2, after the declared-precedence migration | 3,437 | 12,070 | 17,881 | 211,919 |

Inlining cardinality-one attachment units into their owner's rule accounts for
the first step (620 nonterminals, 611 rules, 524 states). Replacing Pratt with
declared precedence accounts for the second (222 nonterminals, 2,263 rules,
2,310 states, 11.4% of the machine).
...
6,578 states contain only token-action kernels. Those two counts were taken on
the v1 machine, where they were 31.8% of 20,715 states; the flattening they
measure is untouched by the declared-precedence migration, and `rule-action.token`
is still 5,469.
...
### 9.2 Expression restrictions: one hand-written `b_expr`, not a clone lattice

This was the second-largest multiplier and is now gone.

Under Pratt, the base `Expr` nonterminal had 179 rules and its 22 distinct
exclusion / minimum-binding-power languages copied another 2,205; 90 one-rule
attachment restrictions and five other attachment restrictions brought the
restriction total to 2,301 rules and 117 nonterminals, accounting exclusively
for 2,415 states (11.7% of the v1 machine). On the declared-precedence Recursa
revision, before the migration, the same lattice still cost 2,296 rules, 112
nonterminals and 2,405 exclusive states.

The seven authored `pratt(exclude(...))` sites were never the large number.
Operand minimum binding powers created the other distinct `Expr` languages, and
cloning every eligible base production turned 22 languages into 2,205 rules.

pg-sql now writes what `gram.y` writes. All seven sites stand for one `gram.y`
nonterminal, `b_expr`, so there is one restricted expression, `BExpr`, declared
by hand as `#[restricts(Expr)]` against `gram.y:15290-15345`:

| Restriction cost | v1 | Step-1 baseline | v2 |
| --- | --: | --: | --: |
| Rules | 2,301 | 2,296 | 144 |
| Nonterminals | 117 | 112 | 1 |
| States (exclusive) | 2,415 | 2,405 | 0 |
| States (mixed with a type kernel) | 147 | — | 10 |

`Expr` itself has 180 rules and `BExpr` 144, against PostgreSQL's hand-factored
`a_expr`, `b_expr` and `c_expr` at 104 rules. The remaining difference is that
PostgreSQL's scanner returns one `Op` token for every operator spelling it does
not name, where pg-sql gives each spelling its own token and its own variant so
that the formatter can reproduce the one it read; all of them are declared at
`gram.y`'s single `Op` level.
...
applications, 452 enums (`Expr` among them, where the v1 machine counted 451
enums plus one Pratt type) and 26 content tokens. Start rules are the sole
kernel origin in 3,007 states, unchanged by the migration. A diagnostic build retaining only
...
Source materialization produces 3,041 normalized types, against 3,038 on the
v1 machine:

| Shape | v1 count | v2 count |
| --- | --: | --: |
| Token | 603 (26 become nonterminals) | 603 |
| Sequence | 1,983 | 1,986 |
| Enum | 451 | 452 |
| Pratt | 1 | 0 |

The Pratt shape is retired: `Expr` is now one of the 452 enums, with 180 rules
where the Pratt body had 179. Token types still contribute 5,469 rules. The
1,986 sequences are accounted for by the public Node applications, the
fixed-token attachment units that are not cardinality one (those now lower
inline into their owner's rule), the presence envelopes and the fixed
sequences. There are 1,478 materialized Node applications against 1,462
authored declarations, so generic monomorphization contributes only 16
applications. The granularity is predominantly deliberate typed-AST factoring,
not accidental generic multiplication.
...
exclusive states, and 1,986 sequence types reflect typed applications,
...
The largest multiplier is now a single lowering choice over that surface:
flattening 26 shared token-admission sets creates 5,469 token rules.
PostgreSQL's handwritten grammar shares keyword categories instead. The second
multiplier, cloning 22 closely related `Expr` languages into 2,205 restriction
rules, is gone: pg-sql maintains the explicit `a_expr`/`b_expr` pair that
PostgreSQL maintains, at 324 rules for the two. Even after removing the
...
the structural difference. That counterfactual was run on the v1 machine.
...
| --- | --: | --: |
| Concrete grammar types | 1,735 | 10,028 |
| Public starts | 1,504 | 1,504 |
| Container helpers and one copy | 197 | 394 |
| Restricted expression (`BExpr`) | 1 | 144 |
| **Total** | **3,437** | **12,070** |
...
gap. Experiments rejected reduction-based lexical categories and duplicate
public-root tables on runtime and binary-size grounds. Pratt restriction
sharing, the remaining structural candidate at the time, was answered instead
by removing Pratt: the restriction lattice is now one hand-written `b_expr`,
and the 1,504 public starts and the flattened token-admission sets are what is
left. The
