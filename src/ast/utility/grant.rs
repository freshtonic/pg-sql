...
pub struct PrivilegeList<'input> {
    #[sep(COMMA)]
    pub items: recursa::ArenaVec1<'input, Privilege<'input>>,
}
...
pub struct FunctionTarget<'input> {
    #[sep(COMMA)]
    pub sigs: recursa::ArenaVec1<'input, crate::ast::ddl::function::DropFunctionTarget<'input>>,
}
...
pub struct LargeObjectTarget<'input> {
    #[sep(COMMA)]
    pub oids: recursa::ArenaVec1<'input, literal::IntegerLit<'input>>,
}
...
pub struct GranteeList<'input> {
    #[sep(COMMA)]
    pub items: recursa::ArenaVec1<'input, Grantee<'input>>,
}
...
#[derive(recursa::Node, Debug)]
#[tok(WITH, this)]
pub struct WithRoleOpts<'input> {
    #[sep(COMMA)]
    pub opts: recursa::ArenaVec1<'input, WithRoleOpt>,
}
...
pub struct GrantRoleBody<'input> {
    ...
    pub with: Option<WithRoleOpts<'input>>,
    pub granted_by: Option<GrantedBy<'input>>,
}
...
pub struct AlterDefaultPrivilegesStmt<'input> {
    pub options: recursa::ArenaVec<'input, DefAclOption<'input>>,
